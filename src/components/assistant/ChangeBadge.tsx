...
export function ChangeBadge({ change }: { change: FieldChange }) {
  ...
  if (field === 'status' || field === 'priority') {
    return (
      <span className="bg-nyx-700 inline-flex items-center gap-1.5 rounded-full px-2 py-1">
        {from != null && (
          <>
            {field === 'status' ? (
  ...
  if (field === 'due') {
    ...
    return (
      <span className="bg-nyx-700 text-nyx-300 inline-flex items-center gap-1.5 rounded-full px-2 py-1 text-caption">
  ...
  return (
    <span className="bg-nyx-700 text-nyx-300 inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-caption">
      <span className="text-nyx-500">{label}</span>
      {from != null && (
        <>
          <span className="text-nyx-600 line-through">{String(from)}</span>
          <span className="text-nyx-600">→</span>
        </>
      )}
