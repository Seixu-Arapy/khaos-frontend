  ...
  return (
    <>
      <button
        ...
        className="border-ink-700 text-ink-400 hover:border-ink-500 hover:text-ink-200 flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-caption transition-colors"
      ...
      >
        <p className="text-ink-400 mb-3 text-body">
        ...
        <input
          ...
          className="border-ink-600 bg-ink-900 text-ink-100 placeholder:text-ink-500 focus:border-copper-400 mb-2 w-full rounded border px-3 py-2 text-body focus:outline-hidden"
        />
        <div className="border-ink-700 max-h-72 overflow-y-auto rounded border">
          {filtered.map((tz) => (
            <button
              ...
              className={`hover:bg-ink-700 flex w-full items-center justify-between px-3 py-2 text-left text-body transition-colors ${
                selected === tz
            ...
            >
              ...
              {selected === tz && (
                <span className="text-copper-500 text-caption">✓</span>
              )}
            </button>
          ))}
          {!filtered.length && (
            <p className="text-ink-600 px-3 py-4 text-body">No results</p>
          )}
        </div>
      </Modal>
    </>
