// Schema discovery core for the chat agent's search_schema tool.
//
// Pure parser over a schema.sql dump — no Vite ?raw import, no browser or
// Deno specifics — so it can be shared by the in-app agent (via
// schemaSearch.ts, which feeds it the bundled schema.sql) and the Telegram
// bot Edge Function (which feeds it its own embedded copy).
...
// Single-entry cache keyed by the SQL string identity — the app passes the
// same schema.sql module on every call, and the Edge Function passes its own
// embedded copy, so each caller reparses at most once.
let cache: { sql: string; parsed: ParsedSchema } | null = null;

function getParsedSchema(schemaSql: string): ParsedSchema {
  if (!cache || cache.sql !== schemaSql) {
    cache = {
      sql: schemaSql,
      parsed: {
...
export function searchSchema(
  ...
  const { enums, tables, functions } = getParsedSchema(schemaSql);
